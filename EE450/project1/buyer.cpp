...
void print_bid_result(UDP_Message &msg){
  ...
  {
    case NO_BIDDING: 
      printf(": Received bidding not open.\n");
      break;
    case ACCEPTED: 
      printf(": Received bid %d$ on %s accepted.\n", msg.price, msg.name);
      break;
    case REJECTED: 
      printf(": Received bid on %s rejected, current bid price %d$.\n", msg.name, msg.price);
      break;
  default:
...
int main(int argc, char *argv[])
{
  ...
  for(int i = 0; i < num_of_entry; i++){
    ...
    print_send_msg(msg);
    while(recvfrom(sockfd, &msg_recv, sizeof(msg_recv), 0, (struct sockaddr*)&recv_addr, &recv_addr_len) <= 0){
