...
private:
    ...
    ListType *hashArray;    // the array of Node pointers, each bucket will have a pointer pointing to the head of the linkedlist
    int numOfEntries;   // number of entries in the table

    //get the length of the longest chain in the table
    int getLongestChain(ListType *hashArray) const;

    //get the number of filled buckets in the table
    int getFilledBuckets(ListType *hashArray) const;
